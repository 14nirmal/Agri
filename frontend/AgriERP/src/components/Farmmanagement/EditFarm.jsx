...
function EditFarm() {
  ...
  const mutation = useMutation({
    ...
    mutationFn: (data, resetForm) => {
      return fetch(`/api/farmer/farm/${farm_id}/editfarm`, {
        method: "PUT",
        credentials: "include",
  ...
  //fetch farm data for the edit

  const { data, isLoading } = useQuery({
    queryKey: ["fetchFarmData"],
    queryFn: async () => {
      const res = await fetch(`/api/farmer/farms?farm_id=${farm_id}`);
        ...
        >
          ...
          {!mutation.error && !mutation.isLoading ? (
            <button
              type="submit"
              disabled={
                (!mutation.error && mutation.isLoading) || !data.isActive
              }
              className={`bg-green-700 mt-2 text-white px-4 py-2 rounded-md font-semibold flex justify-center ${
                !data.isActive && "bg-opacity-55"
              }`}
            >
              {!data.isActive ? "Farm is Disabled" : "Edit Farm"}
            </button>
          ) : (
            <LoadingButton />
          )}
